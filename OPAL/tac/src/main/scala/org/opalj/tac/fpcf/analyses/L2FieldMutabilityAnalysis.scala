...
class L2FieldMutabilityAnalysis private[analyses] (val project: SomeProject) extends FPCFAnalysis {
    ...
    case class State(
        ...
        def hasDependees: Boolean = {
            prematurelyReadDependee.isDefined || purityDependees.nonEmpty ||
                ...
                escapeDependees.nonEmpty || tacDependees.nonEmpty || callerDependees.nonEmpty
        }

        def dependees: Set[SomeEOptionP] = {
            (tacDependees.valuesIterator ++ callerDependees.valuesIterator ++
    ...
    implicit final val typeProvider: TypeProvider = CallGraphKey.typeProvider
    ...
    def getTACAIAndCallers(
        ...
        val tac = propertyStore(method, TACAI.key) match {
            case finalEP: FinalEP[Method, TACAI] ⇒
                finalEP.ub.tac
            case eps: InterimEP[Method, TACAI] ⇒
                state.tacDependees += method → eps
        ...
        val callers = propertyStore(declaredMethod, Callers.key) match {
            case finalEP: FinalEP[DeclaredMethod, Callers] ⇒
                Some(finalEP.ub)
            case eps: InterimEP[DeclaredMethod, Callers] ⇒
                state.callerDependees += declaredMethod -> eps
                state.tacPCs += method → pcs
                Some(eps.ub)
            case epk ⇒
                state.callerDependees += declaredMethod -> epk
                state.tacPCs += method → pcs
                None
        }
